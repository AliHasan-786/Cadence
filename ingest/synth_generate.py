...
EU_COUNTRIES = [
    "DE", "FR", "GB", "ES", "IT", "PL", "NL", "SE", "IE", "BE",
    "AT", "DK", "FI", "PT", "CZ", "GR", "RO", "HU", "BG", "SK",
    "LT", "LV", "EE", "HR", "SI", "LU", "CY", "MT",
]
EU_WEIGHTS = np.array([16, 14, 10, 10, 8, 8, 6, 5, 4, 3,
                       3, 2, 2, 2, 2, 2, 2, 1.5, 1, 1,
                       0.7, 0.5, 0.5, 0.7, 0.5, 0.3, 0.2, 0.1], dtype=float)
...
PLAN_STREAM_MULTIPLIER = {
    "free": 1.0, "individual": 1.6, "duo": 1.8, "family": 2.2, "student": 1.4,
}
...
DISTRIBUTORS = [
    "DistroKid", "TuneCore", "CD Baby", "EMPIRE", "UnitedMasters",
    "Symphonic", "Vydia", "Believe", "Downtown", "ONErpm",
    "STEM", "Too Lost", "Revelator", "AWAL", "Self-Released",
]
DISTRIBUTOR_WEIGHTS = np.array([0.20, 0.14, 0.10, 0.08, 0.06,
                                0.05, 0.04, 0.08, 0.05, 0.04,
                                0.03, 0.03, 0.02, 0.03, 0.05])
DISTRIBUTOR_WEIGHTS = DISTRIBUTOR_WEIGHTS / DISTRIBUTOR_WEIGHTS.sum()

MOD_CATEGORIES = [
    "ANIMAL_WELFARE", "CONSUMER_INFORMATION", "CYBER_VIOLENCE",
    "DATA_PROTECTION", "ILLEGAL_OR_HARMFUL_SPEECH",
    "INTELLECTUAL_PROPERTY_INFRINGEMENTS",
    "NEGATIVE_EFFECTS_CIVIC_DISCOURSE", "NON_CONSENSUAL_BEHAVIOUR",
    "PORNOGRAPHY", "PROTECTION_OF_MINORS",
    "RISK_FOR_PUBLIC_SECURITY", "SCAMS_AND_FRAUD",
    "SELF_HARM", "SCOPE_OF_PLATFORM_SERVICE",
    "UNSAFE_OR_PROHIBITED_PRODUCTS", "VIOLENCE",
]
# IP infringement dominates music platform moderation — Spotify's actual disclosure pattern
MOD_CATEGORY_WEIGHTS = np.array([
    0.005, 0.05, 0.02, 0.02, 0.05,
    0.55,
    0.005, 0.01,
    0.05, 0.05,
    0.005, 0.05,
...
DECISION_TYPES = ["no_action", "remove", "demote", "restrict", "label", "age_gate", "suspend_account"]
...
def _isrc_codes(rng: np.random.Generator, n: int, countries: np.ndarray) -> np.ndarray:
    ...
    return np.array(
        [f"{c}-{s}-{y:02d}-{n:05d}" for c, s, y, n in zip(countries, suffix3_str, yy, nnnnn)]
    )
...
def gen_tracks(n: int, artists: pd.DataFrame) -> pd.DataFrame:
    ...
    duration_ms = np.clip(
        rng.normal(loc=180_000, scale=60_000, size=n), 15_000, 3_600_000
    ).astype(np.int64)
    ...
    release_date = (
        pd.Timestamp(REFERENCE_NOW.date()) - pd.to_timedelta(days_ago, unit="D")
    ).date
...
def gen_appeals(n: int, mod: pd.DataFrame) -> pd.DataFrame:
    ...
    ts_filed = pd.to_datetime(mod_indexed["ts"].to_numpy()) + pd.to_timedelta(file_delay_days, unit="D")
    ...
    reviewer_type[resolved_mask] = rng.choice(["human", "automated"], size=resolved_mask.sum(), p=[0.7, 0.3])
...
def inject_fraud_scenarios(t: Tables) -> dict[str, dict]:
    ...
    bot_users = pd.DataFrame({
        ...
        "country": ["PL"] * 200,
        "plan_type": ["free"] * 200,
        "signup_ts": pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(rng.integers(7, 30, size=200), unit="D"),
        "age_band": ["18-24"] * 200,
        ...
        for j, tid in enumerate(target_tracks):
            for k in range(n_per_track_per_user):
                bot_stream_rows.append((
                    ...
                    uid, tid,
                    pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(int(rng.integers(0, 7 * 86400)), unit="s"),
                    "PL", "android", int(rng.integers(60_000, 180_000)),
                    f"se_{rng.integers(0, 2**48):012x}",
                ))
    bot_streams = pd.DataFrame(
        bot_stream_rows,
        columns=["stream_id", "user_id", "track_id", "ts", "country", "device", "ms_played", "session_id"],
    )
    ...
    ai_release_dates = [
        (REFERENCE_NOW.date() - timedelta(days=int(rng.integers(1, 7))))
        for _ in range(30)
    ]
    ...
    for tid in ai_track_ids:
        for _ in range(n_ai_streams_per_track):
            ...
            ai_stream_rows.append((
                ...
                uid, tid,
                pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(int(rng.integers(0, 7 * 86400)), unit="s"),
                "US", rng.choice(DEVICES, p=DEVICE_WEIGHTS),
    ...
    family_users = pd.DataFrame({
        ...
        "country": ["DE"] * 5,
        "plan_type": ["family"] * 5,
        "signup_ts": pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(rng.integers(30, 365, size=5), unit="D"),
    ...
    for i in range(n_family_streams):
        ...
        fam_rows.append((
            ...
            uid, target_track,
            pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(int(rng.integers(0, 86400)), unit="s"),
            "DE", "speaker", int(rng.integers(120_000, 200_000)),
            f"se_{rng.integers(0, 2**48):012x}",
        ))
    ...
    geo_tracks = pd.DataFrame({
        ...
        "release_date": [REFERENCE_NOW.date() - timedelta(days=int(rng.integers(30, 200))) for _ in range(10)],
    ...
    for i in range(n_geo_jp):
        ...
        geo_rows.append((
            f"s_{sid_base + i:010d}", uid, tid,
            pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(int(rng.integers(0, 30 * 86400)), unit="s"),
            "JP", "android", int(rng.integers(60_000, 180_000)),
            f"se_{rng.integers(0, 2**48):012x}",
        ))
    for i in range(n_geo_us):
        ...
        tid = geo_track_ids[int(rng.integers(0, 10))]
        geo_rows.append((
            f"s_{sid_base + n_geo_jp + i:010d}", uid, tid,
            pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(int(rng.integers(0, 30 * 86400)), unit="s"),
            "US", "ios", int(rng.integers(60_000, 180_000)),
            f"se_{rng.integers(0, 2**48):012x}",
        ))
    ...
    if extra_jp_uids:
        geo_extra_users = pd.DataFrame({
            ...
            "signup_ts": [pd.Timestamp(REFERENCE_NOW) - pd.Timedelta(days=30)] * len(extra_jp_uids),
    ...
    ps_tracks = pd.DataFrame({
        ...
        "isrc": _isrc_codes(rng, 20, np.array(["BG"] * 5 + ["RO"] * 5 + ["VN"] * 5 + ["PH"] * 5)),
        "duration_ms": rng.integers(150_000, 200_000, size=20).astype(np.int64),
        "release_date": [REFERENCE_NOW.date() - timedelta(days=int(rng.integers(7, 60))) for _ in range(20)],
    ...
    start_ts = pd.to_datetime(REFERENCE_NOW) - pd.to_timedelta(int(rng.integers(0, 7 * 86400)), unit="s")
    for i, tid in enumerate(ps_track_ids):
        ps_rows.append((
            f"s_{sid_base + i:010d}", ps_user, tid,
            start_ts + pd.Timedelta(minutes=i * 3),
            "DE", "android", 180_000, ps_session,
        ))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--streams", type=int, default=5_000_000, help="Baseline stream count (default 5M)")
    ...
    for sid, gt in ground_truth.items():
        print(f"  + {sid}: added {gt['n_streams_added']} streams (expected score ≥ {gt['expected_score_min']})")
    ...
    _validate_sample(tables.moderation_actions, ModerationActionRow, VALIDATION_SAMPLE, "moderation_actions")
